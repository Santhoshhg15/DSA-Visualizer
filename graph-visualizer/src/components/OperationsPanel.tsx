  ...
  return (
    <div className="w-full flex flex-col flex-shrink-0">
      <div className="flex items-center justify-between mb-3">
        ...
        <button 
          onClick={handleClear}
          className="text-[10px] font-bold uppercase tracking-[0.04em] bg-red-500/10 border border-red-500/20 px-2 py-1 rounded text-red-400 hover:bg-red-500/20 hover:border-red-500/50 transition-colors"
        >
          Clear Canvas
        </button>
      </div>
          ...
          {[
            ...
            return (
              ...
              >
                ...
                <span className="text-[11px] uppercase font-medium tracking-[0.06em] leading-none">
                  {op.id}
                </span>
              </button>
            );
            ...
            <div className="space-y-3 bg-[var(--input-bg)] p-3 rounded-xl border border-[var(--border-color)]">
              {activeOp === 'Add Vertex' && (
                <div className="mb-3">
                  ...
                  <input 
                    ...
                    placeholder="e.g. Z"
                    className="font-mono w-full h-[36px] bg-[var(--bg-gradient-1)] border border-[var(--border-color)] rounded-[6px] px-3 text-sm text-[var(--text-color)] outline-none focus:border-blue-500 transition-colors"
                  />
                </div>
              )}
              ...
              {(activeOp === 'Add Edge' || activeOp === 'Remove Edge' || activeOp === 'Search Edge') && (
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div>
                    <label className="block text-[10px] font-bold text-[var(--muted-color)] uppercase tracking-[0.06em] mb-1">Source Node</label>
                    <select value={src} onChange={e => setSrc(e.target.value)} className="w-full h-[36px] bg-[var(--bg-gradient-1)] border border-[var(--border-color)] rounded-[6px] px-2 text-sm text-[var(--text-color)] outline-none focus:border-blue-500 transition-colors cursor-pointer appearance-none">
                      <option value="">Select...</option>
                      {nodes.map(n => <option key={n.id} value={n.id}>{n.id}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-[var(--muted-color)] uppercase tracking-[0.06em] mb-1">Target Node</label>
                    <select value={dest} onChange={e => setDest(e.target.value)} className="w-full h-[36px] bg-[var(--bg-gradient-1)] border border-[var(--border-color)] rounded-[6px] px-2 text-sm text-[var(--text-color)] outline-none focus:border-blue-500 transition-colors cursor-pointer appearance-none">
                      <option value="">Select...</option>
                      {nodes.map(n => <option key={n.id} value={n.id}>{n.id}</option>)}
                    </select>
                  </div>
                  {activeOp === 'Add Edge' && graphType.weighted && (
                    <div className="col-span-2 mt-1">
                      <label className="block text-[10px] font-bold text-[var(--muted-color)] uppercase tracking-[0.06em] mb-1">Weight (Optional)</label>
                      <input 
                        type="number" 
                        value={weight} 
              ...
              <button 
                ...
                className="w-full bg-blue-500 hover:bg-blue-400 disabled:bg-[var(--border-color)] disabled:text-[var(--muted-color)] disabled:cursor-not-allowed text-white font-bold h-[36px] rounded-[6px] transition-all text-sm shadow-[0_0_15px_rgba(59,130,246,0.2)] hover:shadow-[0_0_20px_rgba(59,130,246,0.4)] disabled:shadow-none mt-2"
              >
                <span className="text-[11px] font-medium tracking-[0.06em] uppercase">Visualize</span>
              </button>
              ...
              {activeOp === 'Add Vertex' && recentlyAddedVertex && (
                <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-xl animate-fadeInUp">
                  <h3 className="text-[11px] font-bold text-blue-400 uppercase tracking-[0.06em] mb-2 flex justify-between items-center">
                  ...
                  <div className="space-y-2 max-h-[200px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-[var(--border-color)] scrollbar-track-transparent">
                    {nodes.filter(n => n.id !== recentlyAddedVertex).map(n => (
                      <div key={n.id} className="flex items-center gap-2 bg-[var(--panel-bg)] p-2 rounded-[6px] border border-[var(--border-color)] hover:border-blue-500/50 transition-colors">
                        <span className="text-[12px] font-mono font-bold w-6 text-[var(--text-color)]">{n.id}</span>
                        {graphType.weighted && (
                          <input 
                            type="number" 
                            placeholder="W" 
                            className="w-12 h-[24px] bg-[var(--input-bg)] border border-[var(--border-color)] rounded-[4px] px-1 text-[11px] text-center font-mono outline-none focus:border-blue-500 transition-colors text-[var(--text-color)]"
                            id={`weight-${n.id}`}
                          />
                        )}
                        <button 
                          ...
                          className="ml-auto px-2 py-1 bg-blue-500/20 hover:bg-blue-500 text-blue-400 hover:text-white text-[10px] font-bold uppercase tracking-[0.04em] rounded transition-colors"
                        >
                          + Add
                        </button>
                      </div>
                    ...
                    {nodes.length <= 1 && (
                      <p className="text-[11px] text-[var(--muted-color)] text-center py-2">No other vertices to connect to.</p>
                    )}
                  </div>
                </div>
              )}
