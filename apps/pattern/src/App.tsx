...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handlePop = () => {
      ...
      setShowLanding(!p.includes('/visualizer'));
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[var(--bg-gradient-1)] via-[var(--bg-gradient-2)] to-[var(--bg-gradient-3)] text-[var(--text-color)] relative flex flex-col transition-all duration-300">
      ...
      {showLanding ? (
        <PatternLandingPage
          onOpenVisualizer={() => {
            window.history.pushState(null, '', '/pattern/visualizer');
          ...
          onSelectAlgorithm={(algoId) => {
            window.history.pushState(null, '', '/pattern/visualizer');
            window.dispatchEvent(new Event('popstate'));
            useStore.getState().setAlgo(algoId as any);
            setShowLanding(false);
          }}
        />
      ) : (
      ...
      <header className="fixed top-0 left-0 right-0 h-16 border-b border-[var(--border-color)] bg-[var(--panel-bg)] backdrop-blur-md flex items-center justify-between px-6 z-30 transition-all">
        <div className="flex items-center gap-3 relative">
          {!showLanding && (
            <button
              onClick={() => {
                window.history.pushState(null, '', '/pattern');
                window.dispatchEvent(new Event('popstate'));
                setShowLanding(true);
              }}
              className="text-[10px] uppercase tracking-[0.06em] text-[var(--muted-color)] hover:text-[#5ea8ff] transition-colors mr-2"
            >
              ← PATTERN HOME
            </button>
          )}
