...
export const KnapsackVisualizer: React.FC = () => {
  const { steps, cur, knapsackCapacity, knapsackWeights, knapsackValues } = useDPStore();
  ...
  const currentCapacity = step?.knapsackCapacity !== undefined ? step.knapsackCapacity : knapsackCapacity;
  const currentWeights = step?.knapsackWeights || knapsackWeights || [2, 3, 4, 5];
  const currentValues = step?.knapsackValues || knapsackValues || [3, 4, 5, 6];

  // Auto-fit zoom when capacity or items change
  const handleFitScreen = () => {
    ...
    const contentWidth = (currentCapacity + 2) * 55 + 60;
    const contentHeight = (currentWeights.length + 2) * 45;
  ...
  useEffect(() => {
    handleFitScreen();
  }, [knapsackCapacity, knapsackWeights, knapsackValues]);
  ...
  const rowLabels = [
    '∅',
    ...currentWeights.map(
      (w, idx) => `item${idx + 1} (w=${w},v=${currentValues[idx]})`
    ),
  ];
  const colLabels = Array.from({ length: currentCapacity + 1 }, (_, c) => `${c}`);

  // Table matrix fallback
  const tableData = step.dpTable || [new Array(currentCapacity + 1).fill(null)];
  ...
  return (
    ...
    >
      ...
      >
        <span>Unfilled</span>
        <span style={{ color: 'var(--accent-blue)' }}>● Source (dp[i-1][w], dp[i-1][w-wt])</span>
        <span style={{ color: 'var(--accent-indigo)' }}>● Active (dp[i][w])</span>
