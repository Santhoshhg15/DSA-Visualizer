...
export const LcsVisualizer: React.FC = () => {
  const { steps, cur, lcsStr1, lcsStr2 } = useDPStore();
  ...
  const str1 = step?.lcsStr1 || lcsStr1 || 'abcde';
  const str2 = step?.lcsStr2 || lcsStr2 || 'ace';
  ...
  const handleFitScreen = () => {
    ...
    const contentWidth = (str2.length + 2) * 55 + 60;
    const contentHeight = (str1.length + 2) * 45;
  ...
  const tableData = step.dpTable || [new Array(str2.length + 1).fill(null)];
...
export const LCSVisualizer = LcsVisualizer;
