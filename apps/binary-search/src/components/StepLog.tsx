...
import { CheckCircle2, Info, ArrowRightCircle, Sparkles, AlertCircle } from 'lucide-react';

export const StepLog: React.FC = () => {
  const { steps, cur } = useBSStore();
    ...
    >
      ...
      {isDone && (
        ...
        >
          ...
          <span>{currentStep.msg.split('is ')[1] || 'Completed'}</span>
        </div>
      )}
    </div>
  );
